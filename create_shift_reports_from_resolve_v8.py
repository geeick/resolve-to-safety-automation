"""
create_shift_reports_from_resolve_v8.py
...
Run safe review:
    py create_shift_reports_from_resolve_v8.py

Test one shift report only:
    py create_shift_reports_from_resolve_v8.py --only-shift-location "100 Venice Way"

Test one shift report and open SafetyPark after confirmation:
    py create_shift_reports_from_resolve_v8.py --only-shift-location "100 Venice Way" --open-safetypark

Run using already-downloaded CSV:
    py create_shift_reports_from_resolve_v8.py --csv "resolve_scantopay_yesterday.csv"

Run and open SafetyPark after confirmation:
    py create_shift_reports_from_resolve_v8.py --open-safetypark

Date override:
    py create_shift_reports_from_resolve_v8.py --date 2026-06-18
"""
...
def click_safe_save_button(page):
    # Prefer Django admin's ordinary Save button, not Save and Publish.
    for sel in [
        "input[name='_save']",
        "button[name='_save']",
        ...
        try:
            if page.locator(sel).count() > 0:
                page.click(sel)
                page.wait_for_timeout(2000)
                print(f"Clicked save button: {sel}")
                return True
        except Exception:
            pass

    print("Could not find the ordinary Save button. Nothing clicked.")
    return False
...
def create_one_shift_report_guided(page, row, location_id_map):
    ...
    if AUTO_SAVE_EACH_LOCATION:
        print("Web app approved all locations. Saving this location automatically.")
...
def main():
    ...
    if not args.open_safetypark:
        ...
        print(f"py create_shift_reports_from_resolve_v8.py --csv \"{csv_path}\" --date {safe_date(target_date)} --open-safetypark")
        return
