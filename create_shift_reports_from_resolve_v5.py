...
Run safe review:
    py create_shift_reports_from_resolve_v5.py

Test one shift report only:
    py create_shift_reports_from_resolve_v5.py --only-shift-location "100 Venice Way"

Test one shift report and open SafetyPark after confirmation:
    py create_shift_reports_from_resolve_v5.py --only-shift-location "100 Venice Way" --open-safetypark

Run using already-downloaded CSV:
    py create_shift_reports_from_resolve_v5.py --csv "resolve_scantopay_yesterday.csv"

Run and open SafetyPark after confirmation:
    py create_shift_reports_from_resolve_v5.py --open-safetypark

Date override:
    py create_shift_reports_from_resolve_v5.py --date 2026-06-18
"""
...
def login_to_safetypark_app(page):
    ...
    if not email or not password:
        raise ValueError("Missing SafetyPark login username/password in .env")
...
def main():
    ...
    if not args.open_safetypark:
        ...
        print(f"py create_shift_reports_from_resolve_v5.py --csv \"{csv_path}\" --date {safe_date(target_date)} --open-safetypark")
        return
